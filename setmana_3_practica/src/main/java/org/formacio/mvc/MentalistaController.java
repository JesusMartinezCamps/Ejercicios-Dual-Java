...
public class MentalistaController {
	...
	public String intent (@RequestParam int intent, Model model) {
		...
		Boolean intento = esCorrecto(intent, numero);
		model.addAttribute("acierto", intento);
		
		return "joc";
	}
	
	public boolean esCorrecto(int intent, int numberExpected){
		if(intent == numberExpected){
			return true;
		}else{
			return false;
		}
